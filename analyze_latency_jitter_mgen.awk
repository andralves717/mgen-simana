...
END {
  ...
  total_avg_latency = total_avg_latency/last_flow;
  total_avg_jitter  = total_avg_jitter/last_flow; 
  min_seq = max_seq = 2;
  if (nflows != "" && pps != "" && dur != ""){
    total = nflows * (pps*dur+1);
  } else {
    total_en = 1;
  }
  for (i in flows) {
  ...
  if (nflows != "" && pps != "" && size != "" && dur != ""){
    printf "localhost, localhost, %d, %d, %d, %d, %10.2f, %10.2f, %10.2f, %10.2f, %10.2f, %10.2f, %2.2f\n", nflows, pps, size, dur, total_avg_latency, min_lat, max_lat, total_avg_jitter, min_jitt, max_jitt, (total-received)/total;
  } else {
